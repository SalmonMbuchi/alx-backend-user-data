...
def log_in_wrong_password(email: str, password: str) -> None:
    """
    Validate output for wrong password
    """
    r = requests.post('http://localhost:5000/sessions', data={'email': email, 'password': password})
...
def profile_unlogged() -> None:
    """
    Test profile when user is unlogged
    """
...
def profile_logged(session_id: str) -> None:
    """
    Test profile endpoint when user is logged in
    """
    cookie = {'session_id': session_id}
    r = requests.get('http://localhost:5000/profile', cookies=cookie)
    assert r.status_code == 200 
...
def log_out(session_id: str) -> None:
    ...
    """
    cookie = {'session_id': session_id}
    r = requests.delete('http://localhost:5000/sessions', cookies=cookie)
    assert r.status_code == 200
...
def reset_password_token(email: str) -> str:
    ...
    """
    r = requests.post('http://localhost:5000/reset_password', data={'email': email})
    assert r.status_code == 200
